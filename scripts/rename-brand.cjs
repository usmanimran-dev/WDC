  ...
  for (const file of files) {
    ...
    if (fs.statSync(dpath).isDirectory()) {
      ...
      if (
        ...
        !dpath.endsWith('package-lock.json') && !file.includes('rename-brand')
      ) {
        filelist.push(dpath);
      }
    }
...
const map = [
  { search: /https:\/\/webappdevelopersofchicago\.vercel\.app\/og-image\.jpg/g, replace: 'https://v0.dev/placeholder.svg?height=630&width=1200' }, 
  { search: /https:\/\/webappdevelopersofchicago\.vercel\.app\/twitter-image\.jpg/g, replace: 'https://v0.dev/placeholder.svg?height=600&width=1200' }, 
...
const files = walkSync(path.join(__dirname, '..')); // Root is parent of scripts directory
let changed = 0;
